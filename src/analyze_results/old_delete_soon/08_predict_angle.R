...
load(sprintf("%s/kriged.angle.Rda", results.dir))
...
summary.angle.predictions <- summarize_long_term_z(long.term.subset, 
                                                    nlocations)

# 3. PLOT ---------------------------------------------------------------------

plot.angle.predictions <- plot_predictions_on_us_map(summary.angle.predictions, 
                                                      color.palette = "YlGnBu", 
                                                      text.size = 10,
                                                     is.angle = 1)
for (image.type in c("svg","png")) {
  ggsave(sprintf("map_angle_predictions.%s",image.type), 
         plot = plot.angle.predictions, 
         device = image.type, 
         path = sprintf("%s/figures",results.dir),
         scale = 1, 
         width = 10.9, height = 5.4, units = "cm",
         dpi = 300)
}
